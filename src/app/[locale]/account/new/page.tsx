...
import { useRouter } from '@source/navigation'
import Image from 'next/image'
import GameConsoleGIF from '@public/game-console.gif'
import { useTranslations } from 'next-intl'

export const AccountNewPage = () => {
  ...
  return (
    <div className='mx-auto p-6 md:p-11 h-screen max-h-screen min-h-screen w-full flex flex-col justify-center item-center'>
      <div className='md:flex gap-11 h-full w-full'>
        <div className='order-last mx-auto flex-1 max-w-xl flex flex-col justify-center'>
          <h1 className='text-2xl font-bold text-slate-700 sm:text-3xl md:text-4xl text-center'>{translation('signup_title')}</h1>
          <div className='grid grid-cols-6 gap-6 mt-11'>
            ...
            <div className='col-span-6'>
              <Input placeholder={translation('confirmPassword_input')} />
            </div>
            <div className='col-span-6'>
              <button className='bg-blue-500 rounded-2xl font-medium text-white text-center px-4 py-3 w-full'>
                {translation('signup_button')}
              </button>
            </div>
            <div className='col-span-6'>
              <span className='text-sm'>
                {translation.rich('signin_link', {
                  link: (chunks) => (
                    <button onClick={() => router.replace('/account/signin')} className='text-blue-500'>
                      {chunks}
                    </button>
                  )
                })}
